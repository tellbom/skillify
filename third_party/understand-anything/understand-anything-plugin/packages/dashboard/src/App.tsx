...
import { currentProjectId, runtimeUrl } from "./portal-url";
...
function dataUrl(fileName: string, token: string | null): string {
  ...
  return runtimeUrl(`/${fileName}`, token);
}
...
function App() {
  ...
  if (portalMode && currentProjectId() === null) {
    return (
      <I18nProvider language="zh">
        <ThemeProvider>
          <ProjectLauncher accessToken={accessToken} />
        </ThemeProvider>
      </I18nProvider>
    );
  }
