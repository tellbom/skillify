...
Options: `--host <address>` (default `127.0.0.1`),
`--port <n>` (default 5173, auto-increments if taken), `--no-open`.

Everything is served read-only from local disk, bound to `127.0.0.1` by
default, and gated behind an access token — no data leaves your machine.
Production containers can provide the token through
`UNDERSTAND_ACCESS_TOKEN_FILE`, mount graph artifacts separately through
`UNDERSTAND_GRAPH_DIR`, and explicitly listen on `0.0.0.0`.
