 ...
 *
 * Usage:
 *     understand-anything-viewer [project-dir] [--host <address>] [--port <n>] [--no-open]
 *
...
let host = process.env.UNDERSTAND_VIEWER_HOST || "127.0.0.1";
let port = 5173;
...
for (let i = 0; i < args.length; i++) {
  ...
  if (a === "--port") {
    ...
    host = args[++i] ?? "";
    if (!host) {
      console.error("Error: --host requires a listen address");
      process.exit(1);
    }
  } else if (a === "--no-open") {
    openBrowser = false;
  } else if (a === "--help" || a === "-h") {
    console.log("Usage: understand-anything-viewer [project-dir] [--host <address>] [--port <n>] [--no-open]");
    process.exit(0);
...
const configuredGraphDir = process.env.UNDERSTAND_GRAPH_DIR;
const graphDir = configuredGraphDir
  ? path.resolve(configuredGraphDir)
  : UA_DIR_CANDIDATES
      .map((d) => path.join(projectRoot, d))
      .find((d) => fs.existsSync(path.join(d, "knowledge-graph.json")));

if (!graphDir || !fs.existsSync(path.join(graphDir, "knowledge-graph.json"))) {
  console.error(
    `Error: no knowledge graph found under ${projectRoot}\n` +
    "Expected UNDERSTAND_GRAPH_DIR/knowledge-graph.json, .ua/knowledge-graph.json " +
    "(or legacy .understand-anything/). " +
...
function readAccessToken() {
  ...
  if (tokenFile) {
    ...
    if (!token) {
      console.error(`Error: access token file is empty: ${tokenFile}`);
      process.exit(1);
    }
    return token;
  }
...
function listen(attemptPort, attemptsLeft) {
  server.once("error", (err) => {
    if (err.code === "EADDRINUSE" && !portExplicit && attemptsLeft > 0) {
      ...
      console.error(`Error: could not bind ${host}:${attemptPort} — ${err.message}`);
      process.exit(1);
    }
  });
  server.listen(attemptPort, host, () => {
    ...
    const dashboardUrl = `http://${displayHost}:${boundPort}/?token=${ACCESS_TOKEN}`;
