...
— 主子Agent能否被强制指定数量，主子Agent是否通讯正常
...
- 批准的 Shogun 离线 bundle、Linux 宿主依赖和运行期零公网要求。
- 全 OpenCode formation 与全 Claude Code formation 分别完成真实任务。
- Team 单活跃守卫、Worker/模型并发、工作区互斥边界、最小 MCP、网络默认拒绝和凭据引用生效。
- Web、Bridge、DM8、Team/Worker 事件与 Outbox 构成真实闭环，Team 未启用时不影响 `single/delegated`。
- 取消、超时、崩溃、Bridge 重启和清理无遗留进程，事件幂等且无秘密泄漏。

详细销账入口：`docs/testing/2026-shogun-team-e2e-checklist.md`。
...
只有 TP-00 至 TP-13 全部通过，或非适用项有明确的产品裁决和证据，才可完成本轮测试环境交付。
